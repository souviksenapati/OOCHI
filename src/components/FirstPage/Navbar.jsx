...
function Navbar() {
	...
	return (
		<div className='fixed z-10 w-full h-screen px-20 py-8 font-["NeueMontreal"] flex justify-between item-center'>
			<div className="logo">
				<Link to="/">
					<svg
						width="72"
			...
			<div className="links flex gap-10">
				...
				{isOpen ? (
					<FiX className="block transition duration-300 linear delay-700 lg:hidden z-10 " size={45} color="white" onClick={toggleMenu} />
				) : (
					<FiMenu className="block transition duration-300 ease-in delay-300 lg:hidden" size={45} onClick={toggleMenu} />
				)}

				{/* Conditional rendering for the dropdown */}
				{isOpen && (
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="lg:hidden flex absolute top-0 left-0 w-full h-1/2 bg-zinc-900 p-10 text-white flex-col gap-10 ">
							...
							(item, index) => (
								...
								>
									<Link
										...
										className={`text-lg capitalize font-lighter hover:cursor-pointer `}
									>
										{item}
									</Link>
								</motion.span>
					...
					{["Services", "Our Work", "About Us", "Insights", "Contact"].map(
						(item, index) => (
							...
							>
								<Link
									to={`/${item.toLowerCase().replace(/\s+/g, '-')}`}
									className={`text-lg capitalize font-lighter hover:cursor-pointer ${index === 4 && "ml-32"
										}`}
								>
									{item}
								</Link>
